 ...
 */
export async function navigateToIndicatorForm(page: Page, assessmentId: number, indicatorId: number) {
...
export async function clickFirstIndicator(page: Page): Promise<string> {
  ...
  const indicatorCard = page.locator('[data-testid="indicator-card"], a[href*="/indicator/"]').first();
...
export async function saveForm(page: Page) {
  ...
  // Wait for save to complete (look for success toast or button state change)
  await page.waitForTimeout(2000);
}
...
export async function verifyAssessmentStatus(page: Page, expectedStatus: string) {
  ...
  const statusElement = page.locator(`text=/status.*${expectedStatus}/i, [data-status="${expectedStatus}"]`);
...
export async function submitAssessment(page: Page) {
  ...
  const confirmButton = page.locator('button:has-text("Confirm"), button:has-text("Yes"), button:has-text("Submit")').last();
...
export async function resubmitAssessment(page: Page) {
  ...
  // Confirm dialog if present
  const confirmButton = page.locator('button:has-text("Confirm"), button:has-text("Yes"), button:has-text("Resubmit")').last();
  if (await confirmButton.isVisible({ timeout: 2000 }).catch(() => false)) {
    await confirmButton.click();
  }
...
export async function verifyLockedStateBanner(page: Page) {
  const lockedBanner = page.locator('[data-testid="locked-state-banner"], .locked-state-banner, text=/locked|cannot edit|read-only/i').first();
...
export async function verifyReworkCommentsVisible(page: Page) {
  const commentsPanel = page.locator('[data-testid="rework-comments-panel"], text=/assessor feedback|action required/i').first();
