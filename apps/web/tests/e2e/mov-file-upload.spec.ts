...
test.describe('MOV File Upload - Epic 4.0', () => {
  test.beforeAll(async () => {
    ...
    if (!fs.existsSync(imagePath)) {
      ...
      const jpegContent = Buffer.from([0xFF, 0xD8, 0xFF, 0xE0, 0x00, 0x10, 0x4A, 0x46, 0x49, 0x46, 0x00, 0x01, 0x01, 0x00, 0x00, 0x01, 0x00, 0x01, 0x00, 0x00, 0xFF, 0xD9]);
  ...
  test('should preview uploaded file', async ({ page }) => {
    ...
    const previewButton = page.locator('button:has-text("Preview"), button[aria-label="Preview file"]').first();
  ...
  test('should delete uploaded file', async ({ page }) => {
    ...
    // Click delete button
    const deleteButton = page.locator('button:has-text("Delete"), button[aria-label="Delete file"]').first();
  ...
  test('should reject invalid file type', async ({ page }) => {
    ...
    await expect(page.locator('text=File type not supported, text=Invalid file type')).toBeVisible({ timeout: 5000 });
  ...
  test('should reject oversized file', async ({ page }) => {
    ...
    // Verify error message
    await expect(page.locator('text=File size exceeds, text=File too large')).toBeVisible({ timeout: 5000 });
  ...
  test('should display upload progress', async ({ page }) => {
    ...
    const progressIndicator = page.locator('[role="progressbar"], .upload-progress, text=Uploading');

    // Progress should be visible during upload
    await expect(progressIndicator).toBeVisible({ timeout: 2000 }).catch(() => {
