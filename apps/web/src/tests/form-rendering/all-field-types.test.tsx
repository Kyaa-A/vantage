...
describe('All Field Types Rendering', () => {
  it('should render text field with correct attributes', async () => {
    const textFieldDef = allFieldTypesSchema.fields.find(
      (f) => f.name === 'text_field'
    );
  ...
  it('should render number field with min/max constraints', async () => {
    const numberFieldDef = allFieldTypesSchema.fields.find(
      (f) => f.name === 'number_field'
    );
  ...
  it('should render select field with options', async () => {
    const selectFieldDef = allFieldTypesSchema.fields.find(
      (f) => f.name === 'select_field'
    );
  ...
  it('should render radio field with options', async () => {
    const radioFieldDef = allFieldTypesSchema.fields.find(
      (f) => f.name === 'radio_field'
    );
  ...
  it('should render checkbox field with default value', async () => {
    const checkboxFieldDef = allFieldTypesSchema.fields.find(
      (f) => f.name === 'checkbox_field'
    );
  ...
  it('should render file field with accept and maxSize', async () => {
    const fileFieldDef = allFieldTypesSchema.fields.find(
      (f) => f.name === 'file_field'
    );
  ...
  it('should render textarea field with rows', async () => {
    const textareaFieldDef = allFieldTypesSchema.fields.find(
      (f) => f.name === 'textarea_field'
    );
  ...
  it('should render date field', async () => {
    const dateFieldDef = allFieldTypesSchema.fields.find(
      (f) => f.name === 'date_field'
    );
  ...
  it('should correctly mark required vs optional fields', () => {
    ...
    expect(
      requiredFields.find((f) => f.name === 'text_field')
    ).toBeDefined();
    expect(
      requiredFields.find((f) => f.name === 'number_field')
    ).toBeDefined();

    // Verify specific optional fields
    expect(
      optionalFields.find((f) => f.name === 'checkbox_field')
    ).toBeDefined();
    expect(
      optionalFields.find((f) => f.name === 'file_field')
    ).toBeDefined();
  });
  ...
  it('should have valid number field constraints', () => {
    const numberField = allFieldTypesSchema.fields.find(
      (f) => f.name === 'number_field'
    );
  ...
  it('should have valid file field constraints', () => {
    const fileField = allFieldTypesSchema.fields.find(
      (f) => f.name === 'file_field'
    );
  ...
  it('should have valid select/radio options', () => {
    const selectField = allFieldTypesSchema.fields.find(
      (f) => f.name === 'select_field'
    );
    const radioField = allFieldTypesSchema.fields.find(
      (f) => f.name === 'radio_field'
    );
