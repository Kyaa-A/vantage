...
import { useBBIs } from "@/hooks/useBBIs";
import { BBIList } from "@/components/features/bbis";
...
export default function BBIsPage() {
  ...
  // Fetch all BBIs
  const { data: bbisResponse, isLoading, error } = useBBIs();
      ...
      <div className="min-h-screen bg-[var(--background)]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="bg-[var(--card)] border border-[var(--border)] rounded-sm p-6">
            ...
            <p className="text-[var(--muted-foreground)]">
              Failed to load BBIs. Please try refreshing the page.
            </p>
          </div>
        </div>
      </div>
  ...
  const handleCreateNew = () => {
    router.push("/mlgoo/bbis/new");
  };

  const bbis = bbisResponse?.bbis || [];
  const activeBBIs = bbis.filter((bbi) => bbi.is_active);
