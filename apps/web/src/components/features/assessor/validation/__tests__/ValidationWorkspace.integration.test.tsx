...
describe('ValidationWorkspace integration', () => {
  it('Save as Draft calls validate for updated responses', async () => {
  ...
  it('Rework saves draft first then calls rework endpoint', async () => {
    const user = userEvent.setup();
    validateSpy.mockReset();
    reworkSpy.mockReset();
    const assessment = makeAssessment() as any;
  ...
  it('Finalize saves draft first then calls finalize endpoint (no Fail)', async () => {
    const user = userEvent.setup();
    validateSpy.mockReset();
