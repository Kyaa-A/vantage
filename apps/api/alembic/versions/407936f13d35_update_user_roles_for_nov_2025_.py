...
def upgrade() -> None:
    ...
    This migration:
    1. Validates no users have SUPERADMIN role
    2. Renames governance_area_id to validator_area_id
    3. Updates role enum CHECK constraint (removes SUPERADMIN, renames AREA_ASSESSOR to ASSESSOR, adds VALIDATOR)
    4. Migrates existing AREA_ASSESSOR values to ASSESSOR
    5. Sets validator_area_id to NULL for all users with role != VALIDATOR
    """

    # Step 1: Validate no SUPERADMIN users exist
    ...
    # We use execute with raw connection to avoid transaction issues
    connection = op.get_bind()
    ...
    if not result.fetchone():
        # Use raw connection outside transaction for enum modification
    ...
    # This ensures data consistency with the new role structure
    op.execute("UPDATE users SET validator_area_id = NULL WHERE role != 'VALIDATOR'")

    # Note: We cannot remove 'SUPERADMIN' and 'AREA_ASSESSOR' from the enum in PostgreSQL
    # without recreating the enum type entirely. This is safe to leave as they won't be used.
